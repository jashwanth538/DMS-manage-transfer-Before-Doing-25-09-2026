...
import initiateTransfer from "@salesforce/apex/InventoryTransferPending.initiateTransfer";
import initiateTransfers from "@salesforce/apex/InventoryTransferPending.initiateTransfers";
import validateTransferStatus from "@salesforce/apex/InventoryTransferPending.validateTransferStatus";
import validateTransferStatuses from "@salesforce/apex/InventoryTransferPending.validateTransferStatuses";

export default class InventoryTransferPendingPannel extends LightningElement {
  _searchTerm = "";
  ...
  async wiredPendingTransfers(result) {
    this.wiredTransfersResult = result;
    const { data, error } = result;
    if (data) {
      const transfers = data.map((record) => {
        const fromAccName = record.From_Account__r
          ? record.From_Account__r.Name
          : (record.Source_Inventory_Location__r && record.Source_Inventory_Location__r.Account__r
          ? record.Source_Inventory_Location__r.Account__r.Name
          : '-');
        const toAccName = record.To_Account__r
          ? record.To_Account__r.Name
          : (record.Destination_Inventory_Location__r && record.Destination_Inventory_Location__r.Account__r
          ? record.Destination_Inventory_Location__r.Account__r.Name
          : '-');
        ...
        return {
          ...
          Status: record.Status__c || "Requested",
  ...
  get allFilteredTransfers() {
    ...
    // 1. Filter by Location (Source only for outgoing)
    if (this._selectedLocationId && this._selectedLocationId !== "All") {
      data = data.filter(
        (item) => item.SourceLocationId === this._selectedLocationId
      );
    }
    ...
    if (this._searchTerm) {
      ...
      data = data.filter(
        (item) =>
          ...
          (item.FromAccountName && item.FromAccountName.toLowerCase().includes(key)) ||
          (item.ToAccountName && item.ToAccountName.toLowerCase().includes(key)) ||
          (item.SourceLocationName &&
            item.SourceLocationName.toLowerCase().includes(key)) ||
  ...
  get modalTitle() {
    return this.isBulkAction ? "Confirm Bulk Initiation" : "Confirm Initiation";
  }

  get modalActionLabel() {
    return this.isBulkAction
      ? "Initiate Selected Transfers"
      : "Initiate Transfer";
  }
  ...
  async confirmAction() {
    ...
    try {
      ...
      if (isBulk) {
        const validation = await validateTransferStatuses({ transferIds: tIds });
        if (!validation.valid) {
        ...
        const validation = await validateTransferStatus({ transferId: tId });
        if (!validation.valid) {
          this.isLoading = false;
          this.showStaleModal(validation.message);
          return;
      ...
      if (isBulk) {
        const results = await initiateTransfers({ transferIds: tIds });
        this.dispatchEvent(
          new ShowToastEvent({
            title: "Success",
            message: `Successfully initiated ${tIds.length} transfers. Stock moved to In Transit.`,
            variant: "success"
          })
        );
        this.selectedIds = [];
      } else {
        const res = await initiateTransfer({ transferId: tId });
        const transferName = res && res.transferName ? res.transferName : "";
        this.dispatchEvent(
          new ShowToastEvent({
            title: "Success",
            message: `Transfer ${transferName} initiated successfully. Stock moved to In Transit.`,
            variant: "success"
          })
        );
